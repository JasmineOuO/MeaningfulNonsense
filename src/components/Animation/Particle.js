...
export default function Particle(userConfig) {
  ...
  const createParticles = () => {
    ...
    const blowAnimation =
      config.blowAnimations[Math.floor(Math.random() * config.blowAnimations.length)];
    const swayAnimation =
      config.swayAnimations[Math.floor(Math.random() * config.swayAnimations.length)];
    const fallTime = (Math.round(windowHeight * 0.007) + Math.random() * 5) * config.fallSpeed;
    const animation = `fall ${fallTime}s linear 0s 1, ${blowAnimation} ${(fallTime > 30
      ? fallTime
      : 30) -
      20 +
      getRandomInt(0, 20)}s linear 0s infinite, ${swayAnimation} ${getRandomInt(
      2,
      4
